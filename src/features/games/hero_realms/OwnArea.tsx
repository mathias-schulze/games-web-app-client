...
import {
	...
	playerColors,
	PlayerDecksAndCards,
} from './HeroRealmsTable';
import {
	CardType,
	HeroRealmsTableView,
	PlayerArea,
	SpecialActionMode,
} from './HeroRealmsTypes';
...
function OwnArea(props: OwnAreaProps) {
  ...
  const playCard = async (id: string) => {
    await api
      .post(
        HERO_REALMS_ENDPOINT + '/' + props.id + HERO_REALMS_PLAY_CARD_ENDPOINT,
        {cardId: id},
      )
      .then()
      .catch((error) => {});
  ...
  return (
    <Box
      className={classes.ownArea}
      style={{backgroundColor: playerColors[area.position]}}
    >
      <HealthGoldCombatIndicator {...props} observer={props.observer} />
      {area.active && (
        <PlayedCards {...props} area={area} own justifyContent='center' />
      )}
      ...
      {!props.observer &&
        area.actionMode ===
          SpecialActionMode.PUT_CARD_DISCARD_PILE_TOP_DECK && (
          <PutCardTopDeckDialog {...props} onlyChampion={false} />
        )}
      {!props.observer &&
        area.actionMode ===
          SpecialActionMode.PUT_CHAMPION_DISCARD_PILE_TOP_DECK && (
      ...
      {!props.observer &&
        area.actionMode ===
          SpecialActionMode.PUT_CHAMPION_DISCARD_INTO_PLAY && (
      ...
      <Box className={classes.cards}>
        ...
        <Box className={classes.hand}>
          {area.hand.map((handCard) => {
            return (
              <Card
                key={'handCard' + handCard.id}
                alt={handCard.name}
                image={handCard.image}
                onClick={() => {
                  playCard(handCard.id);
                }}
...
export function DiscardCardDialog(props: OwnAreaProps) {
  ...
  return (
    <Dialog open={true} maxWidth='lg' onClose={() => {}}>
      <DialogTitle>Wähle eine Karte zum abwerfen</DialogTitle>
      <DialogContent>
        <Box className={classes.hand}>
...
export function OpponentDiscardCardDialog(props: OwnAreaProps) {
  ...
  const [selected, setSelected] = useState<boolean>(
    props.table.otherPlayerAreas.filter((area) => area.selected4Discard)
      .length > 0,
  );
  ...
  return (
    <Dialog open={true} maxWidth='lg' onClose={() => {}}>
      <DialogTitle>Wähle den Spieler der eine Karte abwerfen muss</DialogTitle>
      <DialogContent>
        <Box className={classes.selectOpponentDeckContainer}>
          {props.table.otherPlayerAreas
            .filter((otherArea) => otherArea.handSize > 0)
            .map((otherArea) => {
              const className = otherArea.selected4Discard
                ? classes.selectOpponentDeckBoxSelected
                : classes.selectOpponentDeckBox;
              return (
                <Box className={className}>
                  <Typography variant='h6'>{otherArea.playerName}</Typography>
                  <Deck
                    alt={otherArea.playerName + ' deck'}
                    count={otherArea.handSize}
                    image={props.table.cardBack}
                    emptyImage={props.table.emptyDeck}
                    onClick={() => {
                      selectPlayer4DiscardCard(otherArea.playerId);
                    }}
                    disabled={selected}
                  />
                </Box>
...
export function PrepareChampionDialog(props: OwnAreaProps) {
  ...
  return (
    <Dialog open={true} maxWidth='lg' onClose={() => {}}>
      <DialogTitle>Wähle einen Champion</DialogTitle>
      <DialogContent>
        <Box className={classes.hand}>
          {props.area.champions
            .filter((champion) => !champion.ready)
            .map((champion) => {
              return (
                <Card
                  key={'championCard' + champion.id}
                  alt={champion.name}
                  image={champion.image}
                  onClick={() => {
                    prepareChampion(champion.id);
                  }}
                  disabled={selected}
                  ready
                />
                  ...
                  {area.champions.map((champion) => {
                    return (
                      <Card
                        ...
                        image={champion.image}
                        onClick={() => {
                          stunTargetChampion(area.playerId, champion.id);
                        }}
                        disabled={selected}
                        ready
                      />
...
export function SacrificeHandOrDiscardDialog(props: OwnAreaProps) {
  ...
  return (
    <Dialog open={true} maxWidth='lg' onClose={() => {}}>
      <DialogTitle>Wähle eine Karte zum opfern</DialogTitle>
      <DialogContent>
        <Box className={classes.selectCardDialog}>
          {props.area.hand.concat(props.area.discardPile.cards).map((card) => {
            return (
              <Card
                key={'sacrificeCard' + card.id}
                alt={card.name}
                image={card.image}
                onClick={() => {
...
export function SelectPlay4BlessOrFireballDialog(
  props: SelectPlay4BlessOrFireballDialogProps,
) {
  const classes = useStyles();
  const [selected, setSelected] = useState<boolean>(false);
  ...
  const onClick = props.bless
    ? selectPlayer4Bless
    : props.fireball
    ? selectPlayer4Fireball
    : () => {};
  const areas = props.bless
    ? allAreas
    : props.fireball
    ? props.table.otherPlayerAreas
    : null;
  ...
  return (
    <Dialog open={true} maxWidth='lg' onClose={() => {}}>
      <DialogTitle>
        {props.bless && <div>Wähle einen Spieler um ihn zu segnen</div>}
        {props.fireball && <div>Wähle einen Spieler als Ziel</div>}
      </DialogTitle>
      <DialogContent>
        <Box className={classes.selectBlessContainer}>
          {areas
            ?.filter((area) => !area.killed)
            .map((area) => {
              return (
                <Box>
                  <Button
  ...
  return (
    <Dialog open={true} maxWidth='lg' onClose={() => {}}>
      ...
      <DialogContent>
        <Box className={classes.hand}>
          {props.area.discardPile.cards
            .filter((champion) => champion.stunnedSinceLastTurn)
            .map((champion) => {
              return (
                <Card
                  key={'championCard' + champion.id}
                  ...
                  image={champion.image}
                  onClick={() => {
                    putChampionIntoPlay(champion.id);
                  }}
                  disabled={selected}
                  ready
                />
            ...
            return (
              <Box className={classes.rangerTrackCardContainer}>
                ...
                <Box>
                  ...
                  <IconButton
                    onClick={() => {
                      rangerTrackDiscard(card.id);
                    }}
                    disabled={area.rangerTrackDiscardCount === 0}
                  >
                    <Delete />
                  </IconButton>
                  {index < cardCount - 1 && (
...
export function AcquireOpponentDiscardDialog(props: OwnAreaProps) {
  ...
  return (
    <Dialog open={true} maxWidth='lg' onClose={() => {}}>
      <DialogTitle>Wähle eine Karte die du kaufen möchtest.</DialogTitle>
      <DialogContent>
        <Box className={classes.stunChampionPlayerBox}>
            ...
            return (
              <Fragment>
                <Typography variant='subtitle1'>
                  {otherArea.playerName}
                </Typography>
                <Box className={classes.stunChampionChampionsBox}>
                  {otherArea.discardPile.cards
                    .filter(
                      (card) => card.cost > 0 && card.cost <= props.area.gold,
                    )
                    .map((card) => {
                      return (
                        <Card
                          ...
                          image={card.image}
                          onClick={() => {
                            acquireOpponentCard(otherArea.playerId, card.id);
                          }}
                          disabled={selected}
                          ready
                        />
